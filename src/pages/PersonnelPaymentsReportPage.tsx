...
import { PersonnelPaymentsPrintTable } from '@/components/personnel-payments/PersonnelPaymentsPrintTable';

export default function PersonnelPaymentsReportPage() {
  ...
  const { data: paymentsData = [], isLoading } = usePersonnelPaymentsQuery(
  ...
  const handleBack = () => {
    navigate('/app/pagamentos-avulsos');
  };
  ...
  const payments = paymentsData.map(p => ({
    id: p.id,
    personnelName: p.personnel?.name || 'N/A',
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Estilos para tela e impressão - copiados da PayrollReportPage */}
      <style>{`
        @media screen {
      ...
      <div className="no-print sticky top-0 z-10 bg-background border-b border-border p-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-semibold">Relatório de Pagamentos Avulsos</h1>
        </div>
