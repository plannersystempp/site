-- ORDEM 1: CRIAR A FUNÇÃO DE "SEEDING"
-- Esta função será responsável por inserir a lista de funções padrão para uma nova equipe.

CREATE OR REPLACE FUNCTION public.seed_default_functions()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER -- Executa com os privilégios do criador da função, necessário para inserir dados
AS $$
BEGIN
...
-- ORDEM 2: CRIAR O TRIGGER
-- Este trigger irá chamar a função 'seed_default_functions' automaticamente
-- sempre que uma nova linha for inserida na tabela 'teams'.

-- Primeiro, remove qualquer trigger antigo com o mesmo nome para evitar erros.
DROP TRIGGER IF EXISTS on_new_team_created ON public.teams;

-- Cria o novo trigger
CREATE TRIGGER on_new_team_created
  AFTER INSERT ON public.teams
  FOR EACH ROW
  EXECUTE FUNCTION public.seed_default_functions();