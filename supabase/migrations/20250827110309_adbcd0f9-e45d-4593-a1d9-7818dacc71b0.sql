-- Phase 1: Critical Database Integrity Improvements (Fixed)
...
-- Add performance indexes (removed CONCURRENTLY for transaction compatibility)
CREATE INDEX IF NOT EXISTS idx_personnel_allocations_event_personnel ON public.personnel_allocations (event_id, personnel_id);
CREATE INDEX IF NOT EXISTS idx_personnel_allocations_division ON public.personnel_allocations (division_id);
CREATE INDEX IF NOT EXISTS idx_work_records_event_employee ON public.work_records (event_id, employee_id);
CREATE INDEX IF NOT EXISTS idx_work_records_date ON public.work_records (work_date);
CREATE INDEX IF NOT EXISTS idx_freelancer_ratings_freelancer ON public.freelancer_ratings (freelancer_id);
CREATE INDEX IF NOT EXISTS idx_audit_logs_created_at ON public.audit_logs (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_audit_logs_team_action ON public.audit_logs (team_id, action);
