-- Backfill de funções padrão para equipes existentes
-- Este script insere as funções padrão apenas para equipes que ainda não as têm.
...
WHERE
    -- A condição chave: só insere se a equipe não tiver nenhuma função ainda.
    NOT EXISTS (
        SELECT 1
