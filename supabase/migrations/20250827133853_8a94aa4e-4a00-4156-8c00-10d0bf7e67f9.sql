...
-- 1) Corrigir a função de auditoria para não fazer cast de bigint -> uuid
...
BEGIN
  ...
  INSERT INTO public.audit_logs (
    ...
    TG_OP,
    TG_TABLE_NAME,
    COALESCE(NEW.id::text, OLD.id::text),  -- IMPORTANTE: registrar como texto
...
-- 2) Garantir trigger de defaults/validação em absences
...
BEGIN
  ...
  -- Valida se a data pertence aos work_days da alocação (comparação textual)
  SELECT EXISTS (
    SELECT 1
    FROM public.personnel_allocations pa
    WHERE pa.id = NEW.assignment_id
...
-- 3) Recriar trigger de auditoria para absences (elimina possíveis versões antigas)
DROP TRIGGER IF EXISTS audit_absences ON public.absences;
DROP TRIGGER IF EXISTS audit_log_absences ON public.absences;
